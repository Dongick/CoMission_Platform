...
public interface MissionRepository extends MongoRepository<MissionDocument, ObjectId> {
    @Query(value= "{status: {$ne: 'COMPLETED'}}", sort = "{createdAt: -1}", fields="{id:1, username:1, creatorEmail:1, title:1, minParticipants:1, participants:1, duration:1, status:1, frequency:1}")
    Page<MissionInfo> findAllAndStatusNotByOrderByCreatedAtDesc(Pageable pageable);

    @Query(value= "{_id: {$in: ?0}, status: {$ne: 'COMPLETED'}}", sort = "{createdAt: -1}", fields="{id:1, username:1, creatorEmail:1, title:1, minParticipants:1, participants:1, duration:1, status:1, frequency:1}")
    ...
    @Query(value = "{title: {$regex: ?0, $options: 'i'}, status: {$ne: 'COMPLETED'}}", sort = "{createdAt: -1}", fields = "{id:1, username:1, creatorEmail:1, title:1, minParticipants:1, participants:1, duration:1, status:1, frequency:1}")
